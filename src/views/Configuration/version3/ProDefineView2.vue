        ...
        <div v-for="(option, index) in options" :key="index" class="process-item">
          <div class="name-delete">
            <div class="id-name-row" style="align-items: baseline;">
              ...
              <input v-model="option.lowChoiceNum" placeholder="Low Choice Number" class="choice-number-input form-control" />
              <label class="ms-2">to</label>
              <input v-model="option.upChoiceNum" placeholder="Up Choice Number" class="choice-number-input form-control" />
            </div>
            <div>
...
const dsReplenish = () => {
  ...
  options.value.forEach((option, optIndex) => {
    let cardDeclare = `(props: Option${optIndex + 1}(1..${option.properties.length}), n: ${option.lowChoiceNum}..${option.upChoiceNum})`;
    if (optIndex == options.value.length - 1) {
          ...
          if(property.attributes.length > 0) {
            property.attributes.forEach((attr, attrIndex) => {
              if (attribute.name === attr.name && attributeIndex === attrIndex) {
                if (optIndex === options.value.length - 1 && proIndex == option.properties.length - 1) {

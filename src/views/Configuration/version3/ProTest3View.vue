...
const customSolutionFilter = () => {
  ...
  customSolutions.value.forEach((solution) => {
    ...
    attributes.value.forEach((attribute, attrIndex) => {
      let attrName = `Total${attribute.name}`;
      dis += parseFloat(distance(solution[attrName], attrMin[attrIndex], attrMax[attrIndex], attribute.index));
      console.log(dis);
    })
    disArray.push(Math.abs(parseFloat(dis) - parseFloat(baseline)).toFixed(3));
  })
...
const distance = (attrValue, minValue, maxValue, attrIndex) => {
  const squareValue = ((attrValue - minValue) / (maxValue - minValue)).toFixed(3);
